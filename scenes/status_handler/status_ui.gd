...
func set_status(new_status: LegalStatus) -> void:
	...
	duration.visible = status.stack_type == LegalStatus.StackType.DURATION
	stacks.visible = status.stack_type == LegalStatus.StackType.INTENSITY
...
func _on_status_changed() -> void:
	...
	if status.stack_type == LegalStatus.StackType.INTENSITY and status.stacks == 0:
		queue_free()
